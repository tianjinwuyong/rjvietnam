/**
 * rda-sop-manager.js — Adaptive SOP Engine for RDA AI Manager
 *
 * Drives the RDA patrol workflow through a state-machine SOP structure.
 *
 * Usage:
 *   node rda-sop-manager.js run --sop rda-sop.json --state rda-sop-state.json [--start step_id]
 *   node rda-sop-manager.js resume --sop rda-sop.json --state rda-sop-state.json
 *   node rda-sop-manager.js next --sop rda-sop.json --state rda-sop-state.json
 *   node rda-sop-manager.js state-set --sop rda-sop.json --state rda-sop-state.json --key <k> --value <v>
 */
...
function log(level, msg) {
  ...
  console.error(`${ts} [${level}] [RDA-SOP] ${msg}`);
}
...
async function notifySopComplete(sopName, cycleId, stepsRun, outcome) {
  await bus.init();
  await bus.send("rda-ai", "sop_cycle_complete", {
    sop_name: sopName,
    cycle_id: cycleId,
    steps_run: stepsRun,
    outcome,
...
async function notifySopStepError(sopName, stepId, errorMsg) {
  await bus.init();
  await bus.send("rda-ai", "system_alert", {
    alert_level: "high",
    source_agent: "rda-sop-ai",
...
async function executeStep(step, results) {
  ...
  switch (step.type) {
    case "query": {
      const script = resolve(PROJECT_ROOT, "services/worker", step.script || "rda-query.js");
    ...
    case "execute": {
      const script = resolve(PROJECT_ROOT, "services/worker", step.script || "rda-execute.js");
    ...
    case "eval": {
      const script = resolve(PROJECT_ROOT, "services/worker", step.script || "rda-evaluator.js");
    ...
    case "notify": {
      const msg = step.params?.message || "RDA SOP cycle complete";
      log("INFO", `Notify: ${msg}`);
...
async function runSop(sopPath, statePath, startStepId) {
  ...
  while (currentStepId && currentStepId !== "END") {
    ...
    try {
      ...
      await notifySopStepError(sop.name || "rda-sop", step.id, err.message);
  ...
  await notifySopComplete(sop.name || "rda-sop", cycleId, stepsRun, outcome);
...
async function main() {
  const sopPath = getArg("sop") || "services/worker/rda-sop.json";
  const statePath = getArg("state") || "services/worker/rda-sop-state.json";
