/**
 * hr-sop-manager.js — Adaptive SOP Engine for HR AI Manager
 *
 * Drives the HR patrol workflow through a state-machine SOP structure.
 *
 * Usage:
 *   node hr-sop-manager.js run --sop hr-sop.json --state hr-sop-state.json [--start step_id]
 *   node hr-sop-manager.js resume --sop hr-sop.json --state hr-sop-state.json
 *   node hr-sop-manager.js next --sop hr-sop.json --state hr-sop-state.json
 *   node hr-sop-manager.js state-set --sop hr-sop.json --state hr-sop-state.json --key <k> --value <v>
 */
...
function log(level, msg) {
  ...
  console.error(`${ts} [${level}] [HR-SOP] ${msg}`);
}
...
async function notifySopComplete(sopName, cycleId, stepsRun, outcome) {
  await bus.init();
  await bus.send("hr-ai", "sop_cycle_complete", {
    sop_name: sopName,
    cycle_id: cycleId,
    steps_run: stepsRun,
    outcome,
...
async function notifySopStepError(sopName, stepId, errorMsg) {
  await bus.init();
  await bus.send("hr-ai", "system_alert", {
    alert_level: "high",
    source_agent: "hr-sop-ai",
...
async function executeStep(step, results) {
  ...
  switch (step.type) {
    case "query": {
      const script = resolve(PROJECT_ROOT, "services/worker", step.script || "hr-query.js");
    ...
    case "execute": {
      const script = resolve(PROJECT_ROOT, "services/worker", step.script || "hr-execute.js");
    ...
    case "eval": {
      const script = resolve(PROJECT_ROOT, "services/worker", step.script || "hr-evaluator.js");
    ...
    case "notify": {
      const msg = step.params?.message || "HR SOP cycle complete";
      log("INFO", `Notify: ${msg}`);
...
async function runSop(sopPath, statePath, startStepId) {
  ...
  while (currentStepId && currentStepId !== "END") {
    ...
    try {
      ...
      await notifySopStepError(sop.name || "hr-sop", step.id, err.message);
  ...
  await notifySopComplete(sop.name || "hr-sop", cycleId, stepsRun, outcome);
...
async function main() {
  const sopPath = getArg("sop") || "services/worker/hr-sop.json";
  const statePath = getArg("state") || "services/worker/hr-sop-state.json";
