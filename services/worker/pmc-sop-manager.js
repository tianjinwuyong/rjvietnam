/**
 * pmc-sop-manager.js — Adaptive SOP Engine for PMC AI Manager
 *
 * Drives the PMC patrol workflow through a state-machine SOP structure.
 * Each step has a type (query, eval, execute, condition, notify, set-variable) and
 * transitions. Steps can be skipped based on results.
 *
 * Usage:
 *   node pmc-sop-manager.js run --sop pmc-sop.json --state pmc-sop-state.json [--start step_id]
 *   node pmc-sop-manager.js resume --sop pmc-sop.json --state pmc-sop-state.json
 *   node pmc-sop-manager.js next --sop pmc-sop.json --state pmc-sop-state.json
 *   node pmc-sop-manager.js state-set --sop pmc-sop.json --state pmc-sop-state.json --key <k> --value <v>
 */
...
function log(level, msg) {
  ...
  console.error(`${ts} [${level}] [PMC-SOP] ${msg}`);
}
...
async function notifySopComplete(sopName, cycleId, stepsRun, outcome) {
  await bus.init();
  await bus.send("pmc-ai", "sop_cycle_complete", {
    sop_name: sopName,
    cycle_id: cycleId,
    steps_run: stepsRun,
    outcome,
...
async function notifySopStepError(sopName, stepId, errorMsg) {
  await bus.init();
  await bus.send("pmc-ai", "system_alert", {
    alert_level: "high",
    source_agent: "pmc-sop-ai",
...
async function executeStep(step, results) {
  ...
  switch (step.type) {
    case "query": {
      const script = resolve(PROJECT_ROOT, "services/worker", step.script || "pmc-query.js");
    ...
    case "execute": {
      const script = resolve(PROJECT_ROOT, "services/worker", step.script || "pmc-execute.js");
    ...
    case "eval": {
      const script = resolve(PROJECT_ROOT, "services/worker", step.script || "pmc-evaluator.js");
    ...
    case "notify": {
      const msg = step.params?.message || "PMC SOP cycle complete";
      log("INFO", `Notify: ${msg}`);
...
async function runSop(sopPath, statePath, startStepId) {
  ...
  while (currentStepId && currentStepId !== "END") {
    ...
    try {
      ...
      await notifySopStepError(sop.name || "pmc-sop", step.id, err.message);
  ...
  await notifySopComplete(sop.name || "pmc-sop", cycleId, stepsRun, outcome);
...
async function main() {
  const sopPath = getArg("sop") || "services/worker/pmc-sop.json";
  const statePath = getArg("state") || "services/worker/pmc-sop-state.json";
  ...
  switch (cmd) {
    ...
    case "resume": {
      ...
      const nextStep = state.__next_step || null;
      await runSop(sopPath, statePath, nextStep);
      break;
    }
    case "next": {
      const state = loadState(statePath);
      const sop = JSON.parse(readFileSync(sopPath, "utf-8"));
      const currentId = state.__current_step || sop.startStep;
      const stepMap = {};
      for (const s of sop.steps) stepMap[s.id] = s;
      const nextId = stepMap[currentId]?.next;
