/**
 * plant-sop-manager.js — Adaptive SOP Engine for Plant AI Manager
 *
 * Drives the Plant-level patrol workflow through a state-machine SOP structure.
 * Coordinates all 6 sub-managers (MES, WMS, BOM, PMC, HR, RDA) each cycle.
 *
 * Usage:
 *   node plant-sop-manager.js run --sop plant-sop.json --state plant-sop-state.json [--start step_id]
 *   node plant-sop-manager.js resume --sop plant-sop.json --state plant-sop-state.json
 *   node plant-sop-manager.js next --sop plant-sop.json --state plant-sop-state.json
 *   node plant-sop-manager.js state-set --sop plant-sop.json --state plant-sop-state.json --key <k> --value <v>
 */
...
function log(level, msg) {
  ...
  console.error(`${ts} [${level}] [PLANT-SOP] ${msg}`);
}
...
async function notifySopComplete(sopName, cycleId, stepsRun, outcome) {
  await bus.init();
  await bus.send("plant-ai", "sop_cycle_complete", {
    sop_name: sopName,
    cycle_id: cycleId,
    steps_run: stepsRun,
    outcome,
...
async function notifySopStepError(sopName, stepId, errorMsg) {
  await bus.init();
  await bus.send("plant-ai", "system_alert", {
    alert_level: "high",
    source_agent: "plant-sop-ai",
...
async function executeStep(step, results) {
  ...
  switch (step.type) {
    case "query": {
      const script = resolve(PROJECT_ROOT, "services/worker", step.script || "plant-query.js");
    ...
    case "execute": {
      const script = resolve(PROJECT_ROOT, "services/worker", step.script || "plant-execute.js");
    ...
    case "eval": {
      const script = resolve(PROJECT_ROOT, "services/worker", step.script || "plant-evaluator.js");
    ...
    case "notify": {
      const msg = step.params?.message || "Plant SOP cycle complete";
      log("INFO", `Notify: ${msg}`);
...
async function runSop(sopPath, statePath, startStepId) {
  ...
  while (currentStepId && currentStepId !== "END") {
    ...
    try {
      ...
      await notifySopStepError(sop.name || "plant-sop", step.id, err.message);
  ...
  await notifySopComplete(sop.name || "plant-sop", cycleId, stepsRun, outcome);
...
async function main() {
  const sopPath = getArg("sop") || "services/worker/plant-sop.json";
  const statePath = getArg("state") || "services/worker/plant-sop-state.json";

  switch (cmd) {
    case "run": {
      await runSop(sopPath, statePath, getArg("start") || null);
      break;
    }
    case "resume": {
