...
export type IqcFlowPageKey =
  ...
  | "reworkComplete" | "iqcReinspection";
...
export function WmsIqcFlowPages({ page, locale: _locale }: { page: IqcFlowPageKey; locale: Locale }) {
  ...
  const endpoint = useMemo(() => page === "defectArchive" ? "/quality/iqc-defect-cases" : page === "mrbApproval" ? "/quality/iqc-mrb-tasks" : page === "iqcReinspection" ? "/quality/iqc-reinspections" : "/wms/receiving/qr-bindings", [page]);
  ...
  return <div className="screen-stack"><section className="surface-panel"><div className="section-header"><div><h2>{info.title}</h2><p>{info.description}</p></div><BackButton /></div>
    ...
    <div className="table-shell" style={{ marginTop: 16 }}><table><thead><tr>{["ID", "单据/档案", "批次", "状态/来源", "数量", "操作"].map(header => <th key={header}>{header}</th>)}</tr></thead><tbody>{rows.length === 0 ? <tr><td colSpan={6}>暂无记录</td></tr> : rows.map(row => <tr key={row.id}><td>{value(row, "id", "material_lot_id")}</td><td>{value(row, "document_no", "case_no", "task_no", "reinspection_no")}</td><td>{value(row, "lot_no")}</td><td>{value(row, "status", "source_type", "decision")}</td><td>{value(row, "quantity", "defective_qty")}</td><td>{page === "iqcReinspection" && ["OPEN", "PENDING"].includes(String(row.status)) && <><button onClick={() => void submit(`/quality/iqc-reinspections/${row.id}/complete`, { result: "PASS", operator: form.operator })}>PASS</button><button onClick={() => void submit(`/quality/iqc-reinspections/${row.id}/complete`, { result: "FAIL", operator: form.operator })}>FAIL</button></>}{page === "defectArchive" && row.status === "PALLET_REMOVAL_PENDING" && <button onClick={() => void submit(`/quality/iqc-defect-cases/${row.id}/pallet-removal`, { operator: form.operator })}>从栈板移除</button>}</td></tr>)}</tbody></table></div>
  </section></div>;
}
