...
export function WmsMaterialRealtimeFlow({ locale, materialCode, lotNo, boxQr, iqcStatus, locationCode, approvalCount, inspectionCount }: { locale: Locale; materialCode: string; lotNo: string; boxQr: string; iqcStatus: string; locationCode?: string; approvalCount?: number; inspectionCount?: number }) {
  ...
  const navigate = (step: StepKey) => {
    const tabs: Record<StepKey, string> = { receiving: "materialReceiving", qr: "materialReceiving", iqc: "iqc", inventory: "inventory", msd: "msd", usage: "pdaConsumption", alarm: "lifecycle" };
    window.location.href = `/?view=wms&wmsTab=${tabs[step]}${lotNo.trim() ? `&lotNo=${encodeURIComponent(lotNo.trim())}` : ""}${materialCode.trim() ? `&materialCode=${encodeURIComponent(materialCode.trim())}` : ""}`;
  };
  return <section className="surface-panel" aria-label="Real-time material process flow">
    ...
    <div style={{ overflowX: "auto" }}><svg viewBox="0 0 1120 155" width="100%" role="img" aria-label="Material process status map" style={{ minWidth: 820 }}>
      ...
      {steps.map((step, index) => { const done = index < activeIndex; const active = index === activeIndex; const x = 72 + index * 160; const color = step.key === "alarm" && active ? "#dc2626" : active ? "#0891b2" : done ? "#16a34a" : "#94a3b8"; return <g key={step.key} role="button" tabIndex={0} onClick={() => navigate(step.key)} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") navigate(step.key); }} style={{ cursor: "pointer" }}><circle cx={x} cy="55" r={active ? 23 : 19} fill={active ? "#ecfeff" : "#fff"} stroke={color} strokeWidth="4" /><text x={x} y="61" textAnchor="middle" fontSize="17" fontWeight="700" fill={color}>{done ? "✓" : index + 1}</text><text x={x} y="100" textAnchor="middle" fontSize="14" fontWeight={active ? "700" : "500"} fill="#0f172a">{step.label}</text>{active && <text x={x} y="124" textAnchor="middle" fontSize="11" fill={color}>{step.next}</text>}</g>; })}
    </svg></div>
