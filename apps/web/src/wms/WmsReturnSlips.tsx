/**
 * WmsReturnSlips — 退库单管理
 * DRAFT → SUBMITTED → EXECUTING → COMPLETED (no APPROVE step)
 */
...
export function WmsReturnSlips() {
  ...
  const fetchItems = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await docApi.return.list({ status: statusFilter || undefined, pageSize: 50 });
      setItems(res.items);
  ...
  const doAction = async (id: number, verb: string) => {
    try {
      switch (verb) {
        case "submit":   await docApi.return.submit(id); break;
        case "reject":   await docApi.return.reject(id); break;
        case "execute":  await docApi.return.execute(id); break;
        case "complete": await docApi.return.complete(id); break;
        case "cancel":   await docApi.return.cancel(id); break;
      }
      setRefreshKey(k => k + 1);
  ...
  const doDelete = async (id: number) => {
    ...
    try { await docApi.return.delete(id); setRefreshKey(k => k + 1); }
    ...
    <div style={{ padding: "16px" }}>
      <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} style={{ padding: "6px 12px", borderRadius: 6, border: "1px solid #ddd" }}>
          ...
          {["DRAFT","SUBMITTED","EXECUTING","COMPLETED","REJECTED","CANCELLED"].map(s => <option key={s} value={s}>{s}</option>)}
        </select>
            ...
            {items.map(item => (
              <tr key={item.id} style={{ borderBottom: "1px solid #eee" }}>
                ...
                <td style={{ padding: "8px 12px", whiteSpace: "nowrap" }}>
                  ...
                  {item.status === "SUBMITTED" && <button onClick={() => doAction(item.id, "execute")} style={{ background: "none", border: "none", cursor: "pointer", color: "#f39c12" }}><Play size={16}/></button>}
