/**
 * WmsOutboundOrders — 出库单/领料单管理
 * DRAFT → SUBMITTED → APPROVED → EXECUTING → COMPLETED
 */
import { useState, useEffect, useCallback } from "react";
import { Plus, Trash2, CheckCircle, XCircle, Play, CheckSquare } from "lucide-react";
...
export function WmsOutboundOrders() {
  ...
  const fetchItems = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await docApi.outbound.list({ status: statusFilter || undefined, pageSize: 50 });
      setItems(res.items);
  ...
  const doAction = async (id: number, verb: string) => {
    try {
      switch (verb) {
        case "submit":   await docApi.outbound.submit(id); break;
        case "approve":  await docApi.outbound.approve(id); break;
        case "reject":   await docApi.outbound.reject(id); break;
        case "execute":  await docApi.outbound.execute(id); break;
        case "complete": await docApi.outbound.complete(id); break;
        case "cancel":   await docApi.outbound.cancel(id); break;
      }
      setRefreshKey(k => k + 1);
  ...
  const doDelete = async (id: number) => {
    ...
    try { await docApi.outbound.delete(id); setRefreshKey(k => k + 1); }
