...
import {
  Activity, Wifi, WifiOff, Monitor, Cpu, Box, Package, Eye, Wrench,
...
function StationCard({ station, onClick }: { station: StationData; onClick: () => void }) {
  ...
  return (
    <div
      ...
      style={{ borderColor, opacity: isOnline ? 1 : 0.6, cursor: "pointer" }}
      onClick={onClick}
    >
...
export function AutoLineStation(_props: { locale: Locale }) {
  ...
  if (selectedStation) {
    const stationEvents = data?.recentEvents.filter(e => e.stationCode === selectedStation.code) ?? [];
    return (
      <div className="screen-stack">
        <div className="surface-panel" style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          {selectedStation.equipmentCode && (
            <span style={{ fontSize: 12, color: "var(--muted)" }}>{selectedStation.equipmentCode}</span>
          )}
        </div>
        ...
        <div className="surface-panel">
          ...
          {stationEvents.length === 0 ? (
            <div style={{ padding: 24, textAlign: "center", color: "var(--muted)" }}>暂无事件数据</div>
          ) : (
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {stationEvents.slice(0, 30).map(e => (
                  <tr key={e.id}>
